...
#else
#define pixyXC 163 // x-coordinate of center of Pixy2 camera
#define pixyYC 110 // y-coordinate of center of Pixy2 camera
#endif
...
#define MAX_BALL_DIST_THRESHOLD 416 // max distance to ball before out of range
#define MIN_BALL_DIST_THRESHOLD 215 // min distance to ball
...
Lidar  lidarRight(Wire, 0x13, +5);               // right LiDAR
...
void loop() {
    ...
    //         float angleScale = constrain(powf((ballAngle < 180.0f ? ballAngle : 360.0f - ballAngle) / 180.0f, 0.3f) , 0, 1);
    //         // DPRINT(angleScale);
    //         float ballDistanceSpeed = SPEED * constrain(distanceScale * powf((1 - angleScale), 4.0f) + angleScale, 0.2, 1);
    ...
    if (ballAngle == -1) {
        ...
        moveTo(91, 190, 3);
        speed = speed * 1.5;
        // speed = constrain(hypot(x-91, y-244-55)/300, 0, 0.5);
    } else {
        // speed = 0.3;
        if (ballAngle < 90) {
            if (goalAngle - ballAngle < 180) {
                // moveTo(182, 244-70, 2);
                moveAngle = 80;
                speed     = (180 - (goalAngle - ballAngle)) / 180 *2 ;
                speed = speed * (1 - (ballDistance - MIN_BALL_DIST_THRESHOLD) / (MAX_BALL_DIST_THRESHOLD - MIN_BALL_DIST_THRESHOLD));
            } else {
                // moveTo(0, 244-70, 2);
                moveAngle = 250;
                speed = ((goalAngle - ballAngle) - 180) / 180 *2;
                speed = speed * (1 - (ballDistance - MIN_BALL_DIST_THRESHOLD) / (MAX_BALL_DIST_THRESHOLD - MIN_BALL_DIST_THRESHOLD));

            }
        } else if (ballAngle > 270) {
            if (ballAngle - goalAngle < 180) {
                // moveTo(0, 244-70, 2);
                moveAngle = 250;
                speed     = (180 - (ballAngle - goalAngle)) / 180 *2;
                speed = speed * (1 - (ballDistance - MIN_BALL_DIST_THRESHOLD) / (MAX_BALL_DIST_THRESHOLD - MIN_BALL_DIST_THRESHOLD));

            } else {
                // moveTo(182, 244-70, 2);
                moveAngle = 80;
                speed     = ((ballAngle - goalAngle) - 180) / 180 *2;
                speed = speed * (1 - (ballDistance - MIN_BALL_DIST_THRESHOLD) / (MAX_BALL_DIST_THRESHOLD - MIN_BALL_DIST_THRESHOLD));

            }
           // speed = constrain(speed * 2, -1, 1);
        } else {
            moveTo(91, 190, 3);
            speed = speed * 1.5;
        }
...
*/
    ...
    driveBase.setDrive(constrain(speed, 0, 1), moveAngle, rotateCommand);
    // driveBase.setDrive(constrain(ballLineOfSightRate * 10, -0.3, 0.3), 90, rotateCommand, 0);
