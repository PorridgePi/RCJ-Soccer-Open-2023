...
class sk {
    ...
    int begin(int pin, int count) {
        ...
        skconfig.clk_div =
            8; // 80MHx / 8 = 10MHz translates to  0,1 us = 100 ns per count

        rmt_config(&skconfig);
        rc =
            rmt_driver_install(skconfig.channel,
                               0,
                               0); //  rmt_driver_install(rmt_channel_t channel,
                                   //  size_t rx_buf_size, int rmt_intr_num)
    ...
    uint32_t
    breakcolor(uint32_t kleur, uint8_t *r, uint8_t *g, uint8_t *b, uint8_t *w) {
        *g = (kleur >> 24) & 0xff;
        *r = (kleur >> 16) & 0xff;
        *b = (kleur >> 8) & 0xff;
        *w = kleur & 0xff;
    ...
    void color(int     led,
        ...
        // _brightness, brightness, bright); Serial.printf( "rgbw =
        // %d.%d.%d.%d\n---\n", r,g,b,w);
    ...
    void show() {
        int rc;
        // esp_err_t rmt_write_items(rmt_channel_t channel, rmt_item32_t
        // *rmt_item, int item_num, bool wait_tx_done)
