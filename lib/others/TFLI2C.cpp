...
bool TFLI2C::Get_Time(uint16_t &tim, uint8_t adr) {
    ...
    if (!readReg(TFL_TICK_LO, adr))
        return false;
    else
        p_tim[0] = regReply; // Read into `tim` array
    if (!readReg(TFL_TICK_HI, adr))
        return false;
    else
        p_tim[1] = regReply; // Read into `tim` array
    return true;
}
...
bool TFLI2C::Get_Prod_Code(uint8_t *p_cod, uint8_t adr) {
    for (uint8_t i = 0; i < 14; ++i) {
        if (!readReg((0x10 + i), adr))
            return false;
        else
            p_cod[i] = regReply; // Read into product code array
    }
    return true;
}
...
bool TFLI2C::Get_Firmware_Version(uint8_t *p_ver, uint8_t adr) {
    for (uint8_t i = 0; i < 3; ++i) {
        if (!readReg((0x0A + i), adr))
            return false;
        else
            p_ver[i] = regReply; // Read into version array
    }
    return true;
}
...
bool TFLI2C::Set_Frame_Rate(uint16_t &frm, uint8_t adr) {
    ...
    // ... then address the pointer as an array.
    if (!writeReg((TFL_FPS_LO), adr, p_frm[0]))
        return false;
    if (!writeReg((TFL_FPS_HI), adr, p_frm[1]))
        return false;
    return true;
}
...
bool TFLI2C::Get_Frame_Rate(uint16_t &frm, uint8_t adr) {
    uint8_t *p_frm = (uint8_t *) &frm;
    if (!readReg(TFL_FPS_LO, adr))
        return false;
    else
        p_frm[0] = regReply; // Read into `frm` array
    if (!readReg(TFL_FPS_HI, adr))
        return false;
    else
        p_frm[1] = regReply; // Read into `frm` array
    return true;
}
...
void TFLI2C::printStatus() {
    Serial.print("Status: ");
    if (tfStatus == TFL_READY)
        Serial.print("READY");
    else if (tfStatus == TFL_SERIAL)
        Serial.print("SERIAL");
    else if (tfStatus == TFL_HEADER)
        Serial.print("HEADER");
    else if (tfStatus == TFL_CHECKSUM)
        Serial.print("CHECKSUM");
    else if (tfStatus == TFL_TIMEOUT)
        Serial.print("TIMEOUT");
    else if (tfStatus == TFL_PASS)
        Serial.print("PASS");
    else if (tfStatus == TFL_FAIL)
        Serial.print("FAIL");
    else if (tfStatus == TFL_I2CREAD)
        Serial.print("I2C-READ");
    else if (tfStatus == TFL_I2CWRITE)
        Serial.print("I2C-WRITE");
    else if (tfStatus == TFL_I2CLENGTH)
        Serial.print("I2C-LENGTH");
    else if (tfStatus == TFL_WEAK)
        Serial.print("Signal weak");
    else if (tfStatus == TFL_STRONG)
        Serial.print("Signal strong");
    else if (tfStatus == TFL_FLOOD)
        Serial.print("Ambient light");
    else if (tfStatus == TFL_INVALID)
